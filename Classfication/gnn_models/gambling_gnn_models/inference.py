...
# 모델 로드 함수
def load_model(model_path, input_dim, hidden_dim, num_heads, dropout, gambling_weight, device, pooling_temperature=5.0):
    """
    업데이트된 모델 구조로 모델을 로드합니다.
    3개의 GAT 레이어와 레이어 정규화, 가중 풀링을 포함합니다.
    """
    ...
    # 풀링 온도 설정 (가중 풀링용)
    model.pooling_temperature = pooling_temperature
...
def inference(model, data, device):
    ...
    with torch.no_grad():
        ...
        if not hasattr(data, 'batch') or data.batch is None:
            # 단일 그래프에 대한 배치 속성 추가 (모델의 일관성을 위해)
            data.batch = None
...
def main():
    # 설정 값
    model_path = '/home/swlab/Desktop/illegal_gambling-detector/classfication/BAG/Classfication/gnn_models/models/gambling_gat_binary_20250415_003647_best.pt'  # 저장된 모델 경로 (binary 모델 사용)
    file_path = '/home/swlab/Desktop/illegal_gambling-detector/classfication/BAG/data/gnn/inference_json/145.json'  # 추론할 파일 경로
    ...
    gambling_weight = 30.0  # 학습 시와 동일한 gambling_weight 값 사용
    pooling_temperature = 5.0  # 가중 풀링을 위한 온도 파라미터
    
    # 모델 로드 - 업데이트된 load_model 함수 사용
    print('모델 로드 중...')
    model = load_model(model_path, input_dim, hidden_dim, num_heads, dropout, gambling_weight, device, pooling_temperature)
    print('모델 로드 완료')
