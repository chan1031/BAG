...
class GamblingGATModel(nn.Module):
    ...
    def forward(self, data):
        ...
        # 가중 풀링 적용 (PyTorch 내장 함수만 사용하도록 수정)
        if hasattr(data, 'batch') and data.batch is not None:
            # 도박 스코어를 소프트맥스로 정규화하여 가중치로 변환
            node_weights = F.softmax(gambling_scores * self.pooling_temperature, dim=0)
            
            # 각 노드의 특성에 가중치 적용
            weighted_x = x * node_weights.unsqueeze(-1)
            
            # 배치별로 합산 (torch_scatter 대신 PyTorch 내장 함수 사용)
            batch_size = data.batch.max().item() + 1
            x_pooled = torch.zeros(batch_size, x.size(1), device=x.device)
            
            for b in range(batch_size):
                batch_mask = (data.batch == b)
                x_pooled[b] = weighted_x[batch_mask].sum(dim=0)
            
            x = x_pooled
        else:
            # 단일 그래프인 경우
            node_weights = F.softmax(gambling_scores * self.pooling_temperature, dim=0)
            x = (x * node_weights.unsqueeze(-1)).sum(dim=0, keepdim=True)
